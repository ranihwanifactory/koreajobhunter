...
const JobBoard: React.FC<JobBoardProps> = ({ user, targetJobId, onTargetJobConsumed, onLoginRequest }) => {
  ...
  useEffect(() => {
    // Permission check: if security rules require auth, we wait for user
    if (!user) {
      setLoading(false);
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      if (error.code === 'permission-denied') {
        setHasPermission(false);
        console.log("Job access permission denied. Login might be required.");
      } else {
  ...
  if (loading) {
    return (
      <div className="py-8 text-center">
        ...
        <p className="text-sm text-gray-500 mt-2">정보 불러오는 중...</p>
      </div>
    );
  }
  ...
  if (!hasPermission || !user) {
    return (
        <div className="bg-white rounded-2xl p-8 text-center shadow-sm border border-gray-100 mb-8 animate-fade-in">
            <div className="text-brand-500 mb-3 text-4xl">
                <i className="fas fa-lock"></i>
            </div>
            <p className="text-gray-800 font-bold">일자리 정보를 보시려면 로그인이 필요합니다.</p>
            <p className="text-gray-500 text-xs mt-1 mb-4">로그인하시면 실시간 구인 정보를 모두 확인하실 수 있습니다.</p>
            <button 
                onClick={onLoginRequest}
                className="bg-brand-600 text-white px-6 py-2 rounded-full text-sm font-bold shadow-md hover:bg-brand-700 transition-colors"
            >
                로그인 하러가기
            </button>
        </div>
